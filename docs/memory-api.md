...
引擎只做确定性存储：**闭集关系 + SQLite 表 + 硬原语**。不理解中文、不改写三元组、不补边、不 `DELETE` 边或节点（只过期）。
...
| `query(graphId, text, budget, principal, at)` | 字面 FTS + 一跳 + 谓语中文标签。空串空结果 |
